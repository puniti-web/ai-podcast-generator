...
from tts_elevenlabs import text_to_speech_elevenlabs
import time
...
st.markdown("<div class='description'>Generate podcast scripts and voices using IBM Watsonx & ElevenLabs</div>", unsafe_allow_html=True)

# 📦 Form Layout
with st.container():
    with st.form("podcast_form"):
        topic = st.text_input("Enter your podcast topic", placeholder="e.g. The Future of AI")
        generate_voice = st.checkbox("Add voiceover using ElevenLabs")
            ...
            if topic.strip() == "":
                ...
                if generate_voice:
                    with st.spinner("Generating voice using ElevenLabs..."):
                        text_to_speech_elevenlabs(script)
                    st.success("Voice generated successfully!")
                    st.audio("output.wav", format="audio/wav")

# 👣 Footer
st.markdown("<div class='footer'>Built with IBM Watsonx & ElevenLabs | Designed by <b>Puniti Jodhwani</b></div>", unsafe_allow_html=True)
